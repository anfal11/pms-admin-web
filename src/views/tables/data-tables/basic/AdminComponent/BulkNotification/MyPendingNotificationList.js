...
const MyPendingNotificationList = ({myPendingNotificationList}) => {
    ...
    const handleFilter = e => {
        ...
        if (value.length) {
        updatedData = myPendingNotificationList.filter(item => {
            const startsWith =
...
function convertArrayOfObjectsToCSV(array) {
  ...
  const exportPDF = (list) => {
    const doc = new jsPDF()
    doc.text('My Pending Notifications', 14, 10) 
    ...
    return (
        <Card>
            <CardHeader className='border-bottom'>
                <CardTitle tag='h4'>My Pending Campaigns</CardTitle>
                <div>
                    <UncontrolledButtonDropdown className='ml-1'>
                        ...
                        <DropdownMenu right>
                            <DropdownItem className='w-100' onClick={() => downloadCSV(myPendingNotificationList)}>
                                <FileText size={15} />
                                <span className='align-middle ml-50'>CSV</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportToXL(myPendingNotificationList)}>
                                <Grid size={15} />
                                <span className='align-middle ml-50'>Excel</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportPDF(myPendingNotificationList)}>
                                <File size={15} />
            ...
            <CommonDataTable column={column} TableData={searchValue.length ? filteredData : myPendingNotificationList} TableDataLoading={TableDataLoading} />
        </Card>
    )
}

export default MyPendingNotificationList