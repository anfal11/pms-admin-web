...
const PendingPromoList = ({ pendingPromoList, setPendingPromoList, setReset }) => {
        ...
        return MySwal.fire({
            ...
            preConfirm: () => {
                ...
                return useJwt.campgnPromotionAprvRjctDlt(data).then(res => {
                    ...
                    Success(res)
                    console.log(res)
                    setPendingPromoList(pendingPromoList.filter(x => x.id !== promotion_id))
                }).catch(err => {
    ...
    const handleFilter = e => {
        ...
        if (value.length) {
            updatedData = pendingPromoList.filter(item => {
                const startsWith =
    ...
    return (
        <Card>
            <CardHeader className='border-bottom'>
                <CardTitle tag='h4'>Pending Promotions</CardTitle>
                <div>
                    <UncontrolledButtonDropdown className='ml-1'>
                        ...
                        <DropdownMenu right>
                            <DropdownItem className='w-100' onClick={() => downloadCSV(pendingPromoList)}>
                                <FileText size={15} />
                                <span className='align-middle ml-50'>CSV</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportToXL(pendingPromoList)}>
                                <Grid size={15} />
                                <span className='align-middle ml-50'>Excel</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportPDF(pendingPromoList)}>
                                <File size={15} />
            ...
            <CommonDataTable column={column} TableData={searchValue.length ? filteredData : pendingPromoList} TableDataLoading={TableDataLoading} />
        </Card>
    )
}

export default PendingPromoList