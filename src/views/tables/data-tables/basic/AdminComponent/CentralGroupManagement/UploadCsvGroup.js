...
const UploadCsvGroup = memo(({setcustomcodecsvurl, handleFile}) => {

    const [isCancel, setisCancel] = useState(false)
    const [ipAddress, setIPAddress] = useState(Math.floor(100000 + (Math.random() * 900000)))

    const socket = socketIOClient(`${FILE_UPLOAD_BASE_URl}/group-upload`, {
        extraHeaders: {
        ...
        <Fragment>
                <Row className='pb-2'>
                    <Col sm='12' className='text-right mb-1'>
                        <Link to="/central group csv.csv" target="_blank" download>
