...
const ViewDetailsModal = ({ modal, toggleModal, managementInfo, setRefresh, refresh, action }) => {
...
useEffect(() => {
    if (managementInfo.id) {
        useJwt.adContentViewApprovalContent(managementInfo.id).then(res => {
            console.log(res)
            const itemArr = []
...
const handleApproveRejectActions = () => {
    ...
    useJwt.adContentAction({ id:managementInfo.id, action_id: action }).then(res => {
                setRefresh(!refresh)
                Success(res)
    ...
    return (
        <Modal size="lg" isOpen={modal} toggle={toggleModal} style={action === 1 ? {border:'2px solid green'} : action === 2 ? {border:'2px solid red'} : {}} >
             <ModalHeader toggle={toggleModal}>{action === 1 ? 'Approve Ad Management' : action === 0 ? 'Reject Ad Management' : 'Ad Management Details'}</ModalHeader>
            <ModalBody>
                <Row>
                    <Col sm='12'>
                            <Card>
