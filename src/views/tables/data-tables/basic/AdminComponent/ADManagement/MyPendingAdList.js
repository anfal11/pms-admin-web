...
const MyPendingADList = ({myPendingadList}) => {
    ...
    const handleFilter = e => {
        ...
        if (value.length) {
        updatedData = myPendingadList.filter(item => {
            const startsWith =
    ...
    return (
        <Card>
            <CardHeader className='border-bottom'>
                <CardTitle tag='h4'>My Pending ADs</CardTitle>
                <div>
                    <UncontrolledButtonDropdown className='ml-1'>
                        ...
                        <DropdownMenu right>
                            <DropdownItem className='w-100' onClick={() => downloadCSV(myPendingadList)}>
                                <FileText size={15} />
                                <span className='align-middle ml-50'>CSV</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportToXL(myPendingadList)}>
                                <Grid size={15} />
                                <span className='align-middle ml-50'>Excel</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportPDF(myPendingadList)}>
                                <File size={15} />
            ...
            <CommonDataTable column={column} TableData={searchValue.length ? filteredData : myPendingadList} TableDataLoading={TableDataLoading} />
        </Card>
    )
}

export default MyPendingADList