...
useEffect(() => {
    if (campaignInfo.id) {
        useJwt.adCampaignViewApproval(campaignInfo.id).then(res => {
            console.log(res)
            const itemArr = []
...
const handleApproveRejectActions = () => {
    ...
    useJwt.adCampaignAction({ id:campaignInfo.id, action_id: action }).then(res => {
                setRefresh(!refresh)
                Success(res)
    ...
    return (
        <Modal size="lg" isOpen={modal} toggle={toggleModal} style={action === 1 ? {border:'2px solid green'} : action === 2 ? {border:'2px solid red'} : {}} >
             <ModalHeader toggle={toggleModal}>{action === 1 ? 'Approve Ad Campaign' : action === 0 ? 'Reject Ad Campaign' : 'Ad Campaign Details'}</ModalHeader>
            <ModalBody>
                <Row>
                    <Col sm='12'>
                            <Card>
