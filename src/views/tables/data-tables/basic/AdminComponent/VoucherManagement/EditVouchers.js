...
const EditVoucher = () => {
    ...
    const onSubmit = (e) => {
        e.preventDefault()
        const { voucherType, Tier, voucherValue, productId, minExpAmount, voucherValidity, expiryDate, rewardPoint, quota, createdBy, terms, Description, business_id } = userInput
    ...
    return (
        <>
            <Button.Ripple onClick={e => history.push('/AllVouchersADMIN')} color='primary' className='m-1'>
                        ...
                        <Col md='4' >
                            <FormGroup>
                                ...
                                <Select
                                    ...
                                    className='react-select'
                                    classNamePrefix='select'
                                    defaultValue={{ value: userInput.Tier, label: userInput.Tier }}
                                    onChange={(selected) => setUserInput({ ...userInput, Tier: selected.value })}
                                    options={tierList.map(x => { return { value: x.Tier, label: x.Tier } })}
                                />
                            </FormGroup>
                        </Col>
                        <Col md='6' >
