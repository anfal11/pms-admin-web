...
const CreateVoucher = () => {
    ...
    useEffect(async () => {
        ...
        //   await useJwt.getTierList().then(res => {
        //       console.log(res)
        //       setTierList(res.data.data)
        //   }).catch(err => {
        //       console.log(err.response)
        //       Error(err)
        //   }).finally(() => {
    ...
    useEffect(() => {
        if (businessid !== '' && tierList?.length !== 0) {
    ...
    const onSubmit = async (e) => {
        e.preventDefault()
        const { voucherType, voucherValue, productId, minExpAmount, voucherValidity, expiryDate, is_product_voucher, purchaseAmount, rewardPoint, quota, createdBy, terms, Description, business_id, Tier } = userInput
        let { product_voucher_map_id } = userInput
        const membershipid = businessList.find(b => b.id === businessid).pms_merchantid
                    ...
                    <Col md='4' >
                        <FormGroup>
                            ...
                            <Select
                                ...
                                className='react-select'
                                classNamePrefix='select'
                                value={{ value: userInput.Tier, label: userInput.Tier }}
                                onChange={(selected) => setUserInput({ ...userInput, Tier: selected.value })}
                                options={[{ value: 'All', label: 'All' }, ...tierList.map(x => { return { value: x.Tier, label: x.Tier } })]}
                            />
                        </FormGroup>
                    </Col>
                    <Col md='6' >
