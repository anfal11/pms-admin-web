...
import { Eye, File, FileText, Grid, Plus, Share, Trash } from 'react-feather'
...
const PendingList = ({TableDataLoading, pendingServicePointRuleList, tierList, ServiceList}) => {
        ...
        if (value.length) {
            updatedData = pendingServicePointRuleList.filter(item => {
                ...
                const includes =
                    ...
                    item.rule_title.toLowerCase().includes(value.toLowerCase()) ||
    ...
    const column = [
        ...
        },
        {
            ...
            selector: row => {
                return <>
                    {created_by !== row.created_by && <span title="View Details">
                        <Eye size={15}
                            color='green'
                            style={{ cursor: 'pointer' }}
                            onClick={(e) => history.push(`/pendingServicePointRule/${row.id}`)}
                        />
                    </span>}
                </>
            }
    ...
    return (
        <Card>
            <CardHeader className='border-bottom'>
                <CardTitle tag='h4'>Pending Service Point Rules</CardTitle>
                <div>
                    ...
                    <UncontrolledButtonDropdown className='ml-1'>
                        ...
                        <DropdownMenu right>
                            <DropdownItem className='w-100' onClick={() => downloadCSV(pendingServicePointRuleList)}>
                                <FileText size={15} />
                                <span className='align-middle ml-50'>CSV</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportToXL(pendingServicePointRuleList)}>
                                <Grid size={15} />
                                <span className='align-middle ml-50'>Excel</span>
                            </DropdownItem>
                            <DropdownItem className='w-100' onClick={() => exportPDF(pendingServicePointRuleList)}>
                                <File size={15} />
            ...
            <CommonDataTable column={column} TableData={searchValue.length ? filteredData : pendingServicePointRuleList} TableDataLoading={TableDataLoading} />
        </Card>
    )
}
