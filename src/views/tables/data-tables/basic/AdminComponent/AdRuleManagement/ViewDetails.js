...
useEffect(() => {
    if (roleInfo.id) {
        useJwt.adRoleViewApproval(roleInfo.id).then(res => {
            console.log(res)
            const itemArr = []
...
const handleApproveRejectActions = () => {
    ...
    useJwt.adRoleAction({ id:roleInfo.id, action_id: action }).then(res => {
                setRefresh(!refresh)
                Success(res)
    ...
    return (
        <Modal size="lg" isOpen={modal} toggle={toggleModal} style={action === 1 ? {border:'2px solid green'} : action === 2 ? {border:'2px solid red'} : {}} >
             <ModalHeader toggle={toggleModal}>{action === 1 ? 'Approve Ad Role' : action === 0 ? 'Reject Ad Role' : 'Ad Role Details'}</ModalHeader>
            <ModalBody>
                <Row>
                    <Col sm='12'>
                            <Card>
