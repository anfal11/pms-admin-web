...
// ** Store & Actions
import { getData, getProducts, getProductsBySearch } from '../store/actions'
...
import { ChevronDown, Plus, Eye, Edit, Trash, MoreVertical, Search, Image as ImageIcon } from 'react-feather'
import DataTable from 'react-data-table-component'
import {
  Card, CardHeader, CardTitle, Input, Label, Row, Col, Spinner, Badge, Button, UncontrolledDropdown, DropdownToggle,
...
const DataTableServerSide = () => {
  ...
  const serverSideColumns = [
...

    {
      ...
      sortable: true,
      minWidth: '100px',
      selector : (row) => `${window.CURRENCY_SYMBOL} ${(+row.RRP).toFixed(2)} `
    },
    {
      name: `Till Price`,
      sortable: true,
      minWidth: '100px',
      selector : (row) => `${window.CURRENCY_SYMBOL} ${(+row.tillprice).toFixed(2)}`
    },
    {
      name: 'Product Volume',
      minWidth: '100px',
      ...
      cell: row => {
        return (
          <div className='d-flex'>
            ...
            <Eye size={15} color='#2bc871' style={{ cursor: 'pointer' }} onClick={(e) => singleProductDetails(row.productid)} /> &nbsp;&nbsp;
             <a href={`/editproductdetails/${row.productid}`} style={{marginTop:'-3px'}}>
...
*/
  ...
  return (
    <Fragment>
      <Button.Ripple className='ml-2 mb-2' color='primary' tag={Link} to='/addNewProduct'>
        <Plus size={15} />
      ...
      <Card>
        ...
        <Col sm="3" >
            ...
            <FormGroup>
              <Label for="exampleSelect3">Product Image</Label>
              <Select
               // theme={selectThemeColors}
                ...
                options={productimageexistornot}
                isClearable
