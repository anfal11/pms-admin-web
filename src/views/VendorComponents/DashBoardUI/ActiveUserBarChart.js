...
const ActiveUserBarChart = ({ labels, dataCount, datalabel }) => {
    const { colors } = useContext(ThemeColors),
        [skin, setSkin] = useSkin(),
        labelColor = skin === 'dark' ? '#8F86F3' : '#6e6b7b',
        tooltipShadow = '#8F86F3',
        gridLineColor = 'rgba(200, 200, 200, 0.2)',
        successColorShade = ['#8F86F3', '#F77028', '#B513FC', '#2FFC13', '#2E4053', '#8FFD03', '#9B59B6']
            ...
            yAxes: [
                {
                    ...
                    ticks: {
                        stepSize: Math.ceil(Math.max(...dataCount) / 3),
                        min: 0,
                        max: Math.max(...dataCount) + Math.ceil(Math.max(...dataCount) / 3),
                        fontColor: labelColor
                    }
                }
            ]
