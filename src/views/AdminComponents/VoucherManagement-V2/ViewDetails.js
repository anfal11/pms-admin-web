...
const ViewDetailsModal = ({ modal, toggleModal, voucherInfo, setRefresh, refresh, action }) => {
...
useEffect(() => {
    if (voucherInfo.id) {
        useJwt2.getApprovalEntryforVoucher(voucherInfo.id).then(res => {
            console.log(res)
            const itemArr = []
...
const handleApproveRejectActions = () => {
    ...
    useJwt2.pmsVoucherAction({ voucherid:voucherInfo.voucherid, action }).then(res => {
                setRefresh(!refresh)
                Success(res)
