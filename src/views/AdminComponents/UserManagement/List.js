...
const List = ({ isloading, menuSubmenus, TableDataLoading, UserList, history, setRefresh, refresh }) => {
    ...
    const handleDelete = (username, message) => {
        return MySwal.fire({
            title: 'Are you sure?',
            text: message,
            icon: 'warning',
            ...
            preConfirm: () => {
                ...
                return useJwt.adminUserDelete({ username }).then(res => {
                    Success(res)
            ...
            selector: row => {
                ...
                <span title="Delete">
                    <Trash size={15}
                        color='red'
                        style={{ cursor: 'pointer' }}
                        onClick={(e) => handleDelete(row.username, 'Do you want to delete?')}
                    />
                </span>
            </>
            }
